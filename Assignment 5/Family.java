...
class Family {
    ...
    public ArrayList<Integer> getChildrenIDs(int id) {
        ...
        for(Person x : family){
            if(x.getParent1() == id || x.getParent2() == id){
                childrenIDs.add(x.getID());
            }
        }
        return childrenIDs;
    ...
    public int[] getParents(int ID) {
        ...
        for(Person x : family){
            if(x.getID() == ID){
                parents[0] = x.getParent1();
                parents[1] = x.getParent2();
                break;
            }
        }
        return parents;
    ...
    public void printChildren(int parentID,String relationship) {
        ...
        for(Person x : family){
            if(x.getParent1() == parentID || x.getParent2() == parentID){
                printChildren(x.getID(), childRelationship(relationship));
            }
        }
    }
}
